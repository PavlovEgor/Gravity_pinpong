...
struct ellipse_parameters_3D space_next_step(struct ellipse_parameters_3D* prev_ep){
    ...
    double vec_x_in_pc[SPACE_DIM], vec_v_in_pc[SPACE_DIM];
    mat_vec_prod(vec_x_in_pc, invS, vec_x);
    mat_vec_prod(vec_v_in_pc, invS, vec_v);

    next_ep.ep.x_start = vec_x_in_pc[0];
    next_ep.ep.y_start = vec_x_in_pc[1];
    next_ep.ep.vx_start = vec_v_in_pc[0];
    next_ep.ep.vy_start = vec_v_in_pc[1];
    next_ep.ep.v_start = next_ep.v_start;

    next_ep.ep.phi_start = phi_from_xy(next_ep.ep.x_start, next_ep.ep.y_start);
    next_ep.ep.r_start = hypot(next_ep.ep.x_start, next_ep.ep.y_start);
    ...
    double  new_vec_x_pc[SPACE_DIM] = {next_ep.ep.x_finish, next_ep.ep.y_finish, 0}, 
            new_vec_v_pc[SPACE_DIM] = {next_ep.ep.vx_finish, next_ep.ep.vy_finish, 0};

    mat_vec_prod(new_vec_x, S, new_vec_x_pc);
    mat_vec_prod(new_vec_v, S, new_vec_v_pc);
